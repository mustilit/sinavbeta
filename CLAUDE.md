...
- **DiscountCode:** Eğiticinin oluşturduğu indirim kodu (aday paket satın alırken kullanır). Sahiplik kuralı: `createdById === TestPackage.educatorId` zorunlu — başka eğiticinin kodu o pakette geçerli değil. `usedCount` artırma `PurchaseUseCase` transaction'ı içinde race-safe (`updateMany ... lt: maxUses`). %50 üst sınır var.
