...
/**
 * Tünel modülü kopya soru tespiti — eğiticinin YALNIZ tünel soru havuzuyla
 * karşılaştırır (test/yazılı ile karışmaz). Jaccard ≥ %75 → amber uyarı.
 */
export class CheckDuplicateTunnelQuestionUseCase {
  async execute(educatorId: string, content: string, excludeQuestionId?: string | null): Promise<CheckDuplicateResult> {
    ...
    const rows = await prisma.tunnelQuestion.findMany({
      where: {
        tunnel: { educatorId },
