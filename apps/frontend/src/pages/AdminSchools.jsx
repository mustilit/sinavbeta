...
import { School, CalendarDays, Plus, UserCog, Power, Users } from "lucide-react";
...
export default function AdminSchools() {
  ...
  const { data: schools = [], isLoading } = useQuery({ queryKey: ["esinif", "schools"], queryFn: adminSchools.list });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex gap-1 border-b border-slate-200">
        <button type="button" onClick={() => setTab("schools")} className={`px-4 py-2.5 min-h-10 text-sm font-medium border-b-2 -mb-px ${tab === "schools" ? "border-indigo-600 text-indigo-600" : "border-transparent text-slate-600 hover:text-slate-900"}`}>Okullar ({schools.length})</button>
      ...
      {tab === "schools" && (
        <div className="space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 text-slate-500"><School className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>Henüz okul yok.</p></div>
          ) : (
                ...
                <Card key={s.id} className={s.isActive ? "" : "opacity-60"}>
                  <CardContent className="p-5 space-y-3">
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                      <span className="text-xs text-slate-500">Yönetici: {s.adminUsername ? <span className="font-mono text-slate-700">{s.adminUsername}</span> : "atanmadı"}</span>
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => setAssignFor(s)} className="gap-1 text-xs"><UserCog className="w-3.5 h-3.5" /> {s.adminUsername ? "Değiştir" : "Yönetici Ata"}</Button>
      ...
      <Dialog open={!!assignFor} onOpenChange={(o) => !o && setAssignFor(null)}>
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={(e) => { e.preventDefault(); const f = new FormData(e.currentTarget); assignAdmin.mutate({ id: assignFor.id, body: { firstName: f.get("firstName") || undefined, lastName: f.get("lastName") || undefined } }); }} className="space-y-3">
            <p className="text-sm text-slate-500">Yeni bir yönetici hesabı oluşturulur; kullanıcı adı ve geçici şifre üretilir.</p>
            <div className="grid grid-cols-2 gap-3">
