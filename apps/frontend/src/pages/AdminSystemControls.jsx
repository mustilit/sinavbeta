...
export default function AdminSystemControls() {
  ...
  const [minWrittenPriceInput, setMinWrittenPriceInput] = useState("");
  ...
  const handleMinWrittenPriceSave = () => {
    const tl = parseFloat(minWrittenPriceInput);
    if (isNaN(tl) || tl < 0) {
      toast.error("Geçerli bir fiyat giriniz (0 = ücretsiz serbest)");
      return;
    }
    const cents = Math.round(tl * 100);
    setSavingKey("minWrittenPriceCents");
    updateMutation.mutate({ minWrittenPriceCents: cents });
    setMinWrittenPriceInput("");
  };
        ...
        <>
          ...
          <div className="p-5 bg-white border border-slate-200 rounded-xl space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-slate-900">Minimum Yazılı Fiyatı</p>
                <p className="text-sm text-slate-500">
                  Eğiticilerin yazılı paket oluştururken girebileceği en düşük fiyat (0 = ücretsiz serbest).
                  Mevcut değer:{" "}
                  <strong className="text-slate-700">
                    {settings?.minWrittenPriceCents != null
                      ? `${(settings.minWrittenPriceCents / 100).toFixed(2)} ₺`
                      : "0,00 ₺"}
                  </strong>
                </p>
              </div>
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1 max-w-xs">
                ...
                <input
                  ...
                  value={minWrittenPriceInput}
                  onChange={(e) => setMinWrittenPriceInput(e.target.value)}
                  className="w-full pl-7 pr-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>
              <button
                onClick={handleMinWrittenPriceSave}
                disabled={minWrittenPriceInput === "" || savingKey === "minWrittenPriceCents"}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-2"
              >
                {savingKey === "minWrittenPriceCents"
                  ? <><Loader2 className="w-4 h-4 animate-spin" />Kaydediliyor...</>
                  : "Kaydet"}
              </button>
            </div>
