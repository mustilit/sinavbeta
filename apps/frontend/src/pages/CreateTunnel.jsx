...
import { Plus, Trash2, Save, Send, Loader2, ArrowLeft, ArrowRight, Layers } from "lucide-react";
...
function emptyQuestion(optionCount) {
  return {
    content: "",
    mediaUrl: "",
    options: Array.from({ length: optionCount }, () => ({ content: "", isCorrect: false })),
  };
}

/**
 * Tünel oluşturma sihirbazı (eğitici).
 *  - Adım 1: sınav türü + konu + başlık (+ fiyat) → tünel (DRAFT) oluşturulur.
 *  - Adım 2: her katman için 10-seçenekli sorular + önizleme + "Onaya gönder".
 * Tema: backend kuralları (10 seçenek/1 doğru, katman min/max) submit'te zorlanır.
 */
export default function CreateTunnel() {
  ...
  // Tünel gelince katman state'ini kur
  useEffect(() => {
    if (!tunnel) return;
  ...
  const saveMut = useMutation({
    mutationFn: () => tunnelApi.saveQuestions(tunnelId, layers),
    onSuccess: () => toast.success("Sorular kaydedildi"),
    onError: (e) => toast.error(e?.message || "Kaydedilemedi"),
  });

  const submitMut = useMutation({
    mutationFn: async () => {
      await tunnelApi.saveQuestions(tunnelId, layers); // önce kaydet
  ...
  // ─── Adım 1 ───
  if (step === 1) {
    const canCreate = title.trim() && examTypeId && topicId;
    return (
      <div className="mx-auto max-w-2xl px-4 py-6">
        <h1 className="mb-1 text-2xl font-bold text-slate-900">Tünel Oluştur</h1>
        <p className="mb-6 text-sm text-slate-500">Adım 1/2 — Sınav türü, konu ve başlık</p>
        <Card>
        ...
        <Card key={qi}>
          <CardContent className="space-y-2 p-4">
            ...
            <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-2">
              {q.options.map((o, oi) => (
                <label key={oi} className="flex items-center gap-2 rounded-lg border border-slate-200 px-2 py-1.5">
                  <input
                    type="radio"
            ...
            <p className="text-xs text-slate-400">Doğru şıkkı radyo ile işaretleyin ({optionCount} seçenek).</p>
          </CardContent>
        </Card>
      ))}
