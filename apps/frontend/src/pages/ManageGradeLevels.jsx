...
import { Plus, Edit2, Trash2, Layers, Search, X, CalendarDays } from "lucide-react";
...
/** Admin Sınıf (GradeLevel) yönetimi — ManageExamTypes ile aynı yapı. */
export default function ManageGradeLevels() {
  ...
  const { data: gradeLevels = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get("/admin/grade-levels", { params: { activeOnly: "false" } });
  ...
  const filtered = useMemo(() => {
    return gradeLevels.filter((g) => {
      ...
      if (filterStatus === "passive" && g.active) return false;
      if (filterDateFrom) {
  ...
  const openDialog = (g = null) => {
    if (g) {
      setEditing(g);
      setFormData({ name: g.name, description: g.description || "", active: g.active ?? true, icon: g.metadata?.icon ?? "" });
    } else {
      setEditing(null);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Sınıflar</h1>
          <p className="text-slate-500 mt-2">Platformdaki sınıf (kademe) kategorilerini yönet</p>
        </div>
        ...
        <CardContent className="p-4">
          ...
          {hasFilter && (
            <p className="text-xs text-slate-400 mt-2">
              {filtered.length} / {gradeLevels.length} sonuç gösteriliyor
            </p>
          )}
        </CardContent>
      </Card>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-500">
                {hasFilter ? "Filtreyle eşleşen sınıf bulunamadı" : "Henüz sınıf yok"}
              </p>
              {hasFilter && (
            ...
            <div className="divide-y divide-slate-100">
              {filtered.slice((page - 1) * pageSize, page * pageSize).map((g) => (
                <div key={g.id} className="flex items-center justify-between p-6">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-indigo-100 rounded-xl flex items-center justify-center overflow-hidden">
                      {(() => {
                        const Ico = getExamTypeIcon(g.metadata?.icon);
                        if (g.metadata?.icon) return <Ico className="w-6 h-6 text-indigo-600" />;
                        if (g.metadata?.iconUrl) return <img src={g.metadata.iconUrl} alt="" className="w-full h-full object-cover" />;
                    ...
                    <div>
                      ...
                      {g.createdAt && (
                        <p className="text-xs text-slate-400 mt-0.5 flex items-center gap-1">
                          ...
                          {new Date(g.createdAt).toLocaleDateString("tr-TR", { day: "numeric", month: "long", year: "numeric" })}
                        </p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className={`text-sm px-3 py-1 rounded-full ${g.active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>
                      {g.active ? "Aktif" : "Pasif"}
                    </span>
                    <Button variant="ghost" size="sm" onClick={() => openDialog(g)}>
                      <Edit2 className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="sm" className="text-rose-600" onClick={() => setDeleteId(g.id)}>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing ? "Sınıfı Düzenle" : "Yeni Sınıf"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              <Label>Logo</Label>
              <p className="text-xs text-slate-500">Havuzdan uygun bir çizgi logo seçin.</p>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Sınıfı silmek istediğinize emin misiniz?</AlertDialogTitle>
            <AlertDialogDescription>Bu işlem geri alınamaz. Bu sınıfa bağlı içerikler "Genel" olarak görünür.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
